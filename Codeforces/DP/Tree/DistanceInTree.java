...
public class DistanceInTree {
    ...
    private static void dfs(int node, int parent, ArrayList<ArrayList<Integer>> adjList, int k) {
        dp[node][0] = 1;
        ...
        for(int adjNode : adjList.get(node)) {
            ...
            for(int num = 1; num <= (k - 1); num++) {
                 res += (dp[adjNode][num - 1] * dp[node][k - num]);
            }

            for(int num = 1; num <= k; num++) {
                dp[node][num] += dp[adjNode][num - 1];
            }
        }
    ...
    private static int solve(ArrayList<ArrayList<Integer>> adjList, int n, int k) {
        dp = new long[n + 1][k + 1];
        fill(dp, 0);
