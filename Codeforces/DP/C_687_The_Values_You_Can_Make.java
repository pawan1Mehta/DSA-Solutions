...
public class C_687_The_Values_You_Can_Make {
    private static ArrayList<Integer> disjoinSubsetWithSumJAndSumK(int[] coins, int k) {
        /*
            dp[i][s][t] = true

            using only first coins 0...i

            we can select some coins such that:
                sum of all selected coins = s + t
                sum of special group among them = s (the remaining coins sum to t)

            dp[i][s][t] means:
                Using coins 0 to i, we can reach a situation where:
                    one part of the coins sums to s
                    another disjoint part of the coins sums to t
        * */
        ...
        boolean[][][] dp = new boolean[maxNum][maxNum][maxNum];
        ...
        for(int i = 1; i < n; i++) {
            ...
            for(int s = 0; s <= k; s++) {
                for(int t = 0; t <= k; t++) {
                    if(coins[i] <= s) {
                        dp[i][s][t] = dp[i][s][t] | dp[i - 1][s - coins[i]][t];
                    }

                    if(coins[i] <= t) {
                        dp[i][s][t] = dp[i][s][t] | dp[i - 1][s][t - coins[i]];
                    }

                    dp[i][s][t] = dp[i][s][t] | dp[i - 1][s][t];
                }
            }
        }
    ...
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
           ...
           ArrayList<Integer> list = disjoinSubsetWithSumJAndSumK(nums, kk);

           fs.writer().write(list.size() + "\n");
           for(int num : list) {
               fs.writer().write(num + " ");
           }
