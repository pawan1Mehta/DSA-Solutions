package BitManupulation.Possibilities;
...
public class Preparing_Olympiad {
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
            ...
            int[] probDiff = fs.readIntArray(n);
            ...
            for(int i = 0; i < allCombinations; i++) {
                int leastDiffProb = Integer.MAX_VALUE;
                int mostDiffProb = Integer.MIN_VALUE;
                int totalProb = 0;
                int sum = 0;

                for(int j = 0; j < n; j++) {
                    if(((1 << j)&i) != 0) {
                        leastDiffProb = Math.min(leastDiffProb, probDiff[j]);
                        mostDiffProb = Math.max(mostDiffProb, probDiff[j]);
                        totalProb++;
                        sum += probDiff[j];
                    }
                }

                if(totalProb >= 2 && sum >= l && sum <= r && (mostDiffProb - leastDiffProb) >= x) {
                    count++;
                }
            }
