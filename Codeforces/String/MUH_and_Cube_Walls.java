...
public class MUH_and_Cube_Walls {

    private static int countNumSegments(int[] dBears, int[] dElephants) {
        if(dElephants.length == 0) {
            return dBears.length + 1;
        }

        int[] nums = new int[dBears.length + dElephants.length];

        for(int i = 0; i < dElephants.length; i++) {
            nums[i] = dElephants[i];
        }
        for(int i = dElephants.length, j = 0; i < nums.length; i++, j++) {
            nums[i] = dBears[j];
        }

        int[] zArray = zArray(nums);

        int k = dElephants.length;
        int count = 0;

        for(int i = dElephants.length; i < zArray.length; i++) {
            if(zArray[i] >= k) {
                count++;
            }
        }
    ...
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
           ...
           int[] bears = fs.readIntArray(n);
           int[] elephants = fs.readIntArray(w);

           int[] dBears = new int[n - 1];
           int[] dElephants = new int[w - 1];

           for(int i = 1; i < n; i++) {
               dBears[i  - 1] = bears[i] - bears[i - 1];
           }
           for(int i = 1; i < w; i++) {
               dElephants[i  - 1] = elephants[i] - elephants[i - 1];
           }

           int count = countNumSegments(dBears, dElephants);
