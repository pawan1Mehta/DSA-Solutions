...
public class Problem2 {
    ...
    private static void reverseArr(int[] arr, int ti, int tj) {
        for(int i = ti, j = tj; i < j; i++, j--) {
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
    ...
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
            ...
            int[] maxNumI = new int[n];
            maxNumI[n - 1] = n - 1;
            for(int i = n - 2; i >= 0; i--) {
                if(arr[i] < arr[maxNumI[i + 1]]) {
                    maxNumI[i] = maxNumI[i + 1];
                } else {
                    maxNumI[i] = i;
                }
            }
