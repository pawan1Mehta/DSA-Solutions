...
public class Github_Send_the_Fool_Further {

    private static long dfs(int node, int parent, ArrayList<ArrayList<int[]>> adjList) {
        long maxSum = 0;
        for(int[] adjNode : adjList.get(node)) {
            if(adjNode[0] != parent) {
                maxSum = Math.max(maxSum, adjNode[1] + dfs(adjNode[0], node, adjList));
            }
        }
        return maxSum;
    }

    public static void solve(FastScanner fs, int n, ArrayList<ArrayList<int[]>> adjList) throws IOException {
        long maxCost = dfs(0, 0, adjList);

        fs.writer().write(maxCost + " \n");
    }
