...
public class GitHub_The_Tag_Game {

    private static void dfs(int node, ArrayList<ArrayList<Integer>> adjList, int[] dist) {
    ...
    public static void solve(FastScanner fs, int n, int aliceStartPt, int botStartPt, ArrayList<ArrayList<Integer>> adjList) throws IOException {
        int[] dist1 = new int[n];
        Arrays.fill(dist1, -1);
        dist1[aliceStartPt] = 0;
        dfs(aliceStartPt, adjList, dist1);

        int[] dist2 = new int[n];
        Arrays.fill(dist2, -1);
        dist2[botStartPt] = 0;
        dfs(botStartPt, adjList, dist2);

        int maxDist = 0;

        for(int node = 0; node < n; node++) {
            if(dist1[node] > dist2[node]) {
                maxDist = Math.max(maxDist, dist1[node]);
            }
        }

        fs.writer().write((2 * maxDist)+ "\n");
    }
