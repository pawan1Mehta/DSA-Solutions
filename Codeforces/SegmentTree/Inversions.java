...
public class Inversions {
    ...
    public static class SegmentTree {
        ...
        private void update(int v, int tl, int tr, int index) {
            if(tl > tr) {
                return;
            }
            ...
            if(index <= tmid) {
                update(2 * v, tl, tmid, index);
            } else {
                update(2 * v + 1, tmid + 1, tr, index);
            }

            tree[v] = tree[2 * v] + tree[2 * v + 1];
        }

        private int inversion(int v, int tl, int tr, int l, int r) {
            if(l > r) {
                return 0;
            }
            ...
            int leftCount = inversion(2 * v, tl, tmid, l, Math.min(tmid, r));
            int rightCount = inversion(2 * v + 1, tmid+ 1, tr, Math.max(tmid + 1, l), r);

            return leftCount + rightCount;
        }

        public int inversion(int l, int r) {
            return inversion(1, 0, n-1, l, r);
        }
    ...
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
            ...
            for(int i = 0; i < n; i++) {
                int index = nums[i] - 1;
                fs.writer().write(st.inversion(index + 1, n - 1) + "\n");
                st.update(index);
            }
        }
