...
public class First_element_at_least_X {
    ...
    public static class SegmentTree {
        ...
        private int findMinIndex(int v, int tl, int tr, int x) {
            if(tree[v] < x) {
                return -1;
            }
            ...
            int index = findMinIndex(2 * v, tl, tmid, x);

            if(index == -1) {
                return findMinIndex(2 * v + 1, tmid + 1, tr, x);
            }
        ...
        public long findMinIndex(int x) {
            return findMinIndex(1, 0, n - 1, x);
        }
    }
        ...
        while(k-- > 0) {
            ...
            for(int i = 0; i < m; i++) {
                ...
                if(a == 1) {
                    ...
                    b = fs.nextInt();
                    long res = st.findMinIndex(b);
                    fs.writer().write((res == Integer.MAX_VALUE ? "-1" : res) + "\n");
                }
            }
        }
