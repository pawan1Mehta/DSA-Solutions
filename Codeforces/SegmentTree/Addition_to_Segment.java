...
public class Addition_to_Segment {
    ...
    static class SegmentTree {
        ...
        private void applyPendingUpdate(int v, int tl, int tr) {
            if(lazy[v] != 0) {
                tree[v] += lazy[v];
        ...
        private void update(int v, int tl, int tr, int l, int r, int val) {
            applyPendingUpdate(v, tl, tr);
            ...
            if(tl >= l && tr <= r) {
                ...
                update(2 * v, tl, tmid, l, Math.min(tmid, r), val);
                update(2 * v + 1, tmid + 1, tr, Math.max(tmid + 1, l), r, val);

                tree[v] = tree[2 * v] + tree[2 * v + 1];
            }
        }

        private long sum(int v, int tl, int tr, int index) {
            applyPendingUpdate(v, tl, tr);
            ...
            if(index <= tmid) {
                return sum(2 * v, tl, tmid, index);
            } else {
                return sum(2 * v + 1, tmid + 1, tr, index);
            }
        }
