...
public class Number_of_Minimums_on_a_Segment {
    public static class SegmentTree {
        ...
        private void buildTree(int v, int tl, int tr, int[] nums) {
            ...
            tree[v] = getMin(tree[2 * v], tree[2 * v + 1]);
        }
        ...
        private long[] minCount(int v, int tl, int tr, int ql, int qr) {
            if(ql > qr) {
            ...
            long[] leftMin = minCount(2 * v, tl, tmid, ql, Math.min(tmid, qr));
            long[] rightMin = minCount(2 * v + 1, tmid + 1, tr, Math.max(tmid + 1, ql), qr);
        ...
        public long[] minCount(int ql, int qr) {
            return minCount(1, 0, n - 1, ql, qr);
        }
    }
        ...
        while(k-- > 0) {
            ...
            for(int i = 0; i < m; i++) {
                ...
                if(a == 1) {
                    ...
                    long[] res = st.minCount(b, c - 1);
                    fs.writer().write(res[0] + " " + res[1] + "\n");
                }
            }
