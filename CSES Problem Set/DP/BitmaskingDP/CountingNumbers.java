package CSESProblemSet.DP.BitmaskingDP;
...
public class CountingNumbers {
    ...
    public static long noAdjacentDigitsAreSameUtil(int pos, int prevDigit, int leadingZeros, int tight, int[] digits) {
        if(pos == digits.length) {
            return leadingZeros == 1 ? 0 : 1;
        }

        if(dp[pos][prevDigit][leadingZeros][tight] != -1) {
            return dp[pos][prevDigit][leadingZeros][tight];
        }
        ...
        for(int digit = 0; digit <= limit; digit++) {
            if(leadingZeros == 0 && digit == prevDigit) {
                continue;
            }

            int newLeadingZerosCount = (leadingZeros == 1 && digit == 0) ? 1 : 0;
            int newTight = (digit == limit && tight == 1) ? 1 : 0;
            int newPrev = (newLeadingZerosCount == 1) ? 0 : digit;

            countNumbers += noAdjacentDigitsAreSameUtil(pos + 1, newPrev, newLeadingZerosCount, newTight, digits);
        }

        return dp[pos][prevDigit][leadingZeros][tight] = countNumbers;
    }

    public static long getCountNoTwoAdjacentDigitsAreSame(long num) {
        int[] digits = getDigits(num);

        dp = new long[20][10][2][2];
        fill(dp, -1);

        return noAdjacentDigitsAreSameUtil(0, 0, 1, 1, digits);
    }

    public static long countingNumbers(long a, long b) {
        long count1 = 0;
        if(a != 0) {
            count1 = getCountNoTwoAdjacentDigitsAreSame(a - 1);
        }

        long result = getCountNoTwoAdjacentDigitsAreSame(b) - count1;
        if(a == 0) result++;
        return result;
    }
