...
public class RectangleCutting {
    ...
    public static long rectangleCutting(int a, int b) {
        dp = new long[a + 1][b + 1];
        fill(dp, 0);
        ...
        for(int i = 1; i <= a; i++) {
            for(int j = 1; j <= b; j++) {
                ...
                for(int k = 1; k < j; k++) {
                    minCut = Math.min(minCut, 1 + dp[i][k] + dp[i][j - k]);
                }

                for(int k = 1; k < i; k++) {
                    minCut = Math.min(minCut, 1 + dp[k][j] + dp[i - k][j]);
                }
