...
public class ArrayDescription {
    ...
    public static int countArraysUtil(int i, int prevVal, int[] nums, int m) {
        if(i == nums.length) {
            return 1;
        }
        ...
        if(nums[i] != 0) {
            if(prevVal == 0 || Math.abs(prevVal - nums[i]) <= 1) {
                count = countArraysUtil(i + 1, nums[i], nums, m);
            }
        } else {
            for(int j = 1; j <= m; j++) {
                if(prevVal == 0 || Math.abs(prevVal - j) <= 1) {
                    count = (count + countArraysUtil(i + 1, j, nums, m)) % MOD;
                }
            }
        }

        return memo[i][prevVal] = count;
    }

    public static int countArrays(int[] nums, int m) {
        int n = nums.length;
        ...
        for(int i = n - 1; i >= 1; i--) {
            for(int j = 1; j <= m; j++) {
                if(nums[i] == 0) {
                    ...
                    if(Math.abs(nums[i] - j) <= 1) {
                        dp[i][j] = (dp[i][j] + dp[i + 1][nums[i]]) % MOD;
                    } else {
                        dp[i][j] = 0;
                    }
                }
        ...
        return countArraysUtil(0, 0, nums, m);
    }
