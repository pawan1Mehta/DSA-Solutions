...
public class RemovalGame {
   ...
   public static long play(int[] nums) {
       int n = nums.length;
       ...
       for(int len = 2; len <= n; len++) {
           for(int i = 0; i <= (n - len); i++) {
               ...
               // If I take i, opponent play optimally on [i + 1 ... j]
               long takeLeft = nums[i] + Math.min(
                 (i + 2 <= j) ? dp[i + 2][j] : 0,
                 (i + 1 <= j - 1) ? dp[i + 1][j - 1]: 0
               );

               // If I take j, opponent play optimally on [i.... j - 1]
               long takeRight = nums[j] + Math.min(
                   (i + 1 <= j - 1) ? dp[i + 1][j - 1] : 0,
                   (i <= j - 2) ? dp[i][j - 2] : 0
               );
