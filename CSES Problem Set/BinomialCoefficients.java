...
public class BinomialCoefficients {
    ...
    public static long binaryCoefficients(long n, long c, long mod) {
        if(c == 0 || n == c) {
            return 1;
        }
        return (binaryCoefficients(n - 1, c - 1, mod)
                + binaryCoefficients(n - 1, c, mod)) % mod;
    }

    // return nCk%mod (O(n^2))
    public static long binomialCoefficient(int n, int k, int mod) {

        int[][] dp = new int[n + 1][k + 1];

        for (int i = 0; i <= n; i++) {
            /**
             * There is only one way to chose 0 items from a set of i items (don't chose
             * anything)
             */
            dp[i][0] = 1;

            /**
             * There is only one way to chose i items from a set of i items (chose every
             * element in the set)
             */
            if (i <= k) {
                dp[i][i] = 1;
    ...
    public  static void main(String[] args) throws Exception {
        ...
        while(k-- > 0) {
            ...
            long res = binomialCoefficient(n, c, MOD);
