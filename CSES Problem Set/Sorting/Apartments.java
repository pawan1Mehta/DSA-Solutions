...
public class Apartments {
    ...
    private static int distributeApartment(int[] desiredApartmentSize, int[] apartmentSize, int k) {
        int n = desiredApartmentSize.length;
        int m = apartmentSize.length;
        ...
        int i = 0, j = 0;
        while(i < n && j < m) {
            int min = desiredApartmentSize[i] - k, max = desiredApartmentSize[i] + k;

            if(apartmentSize[j] >= min && apartmentSize[j] <= max) {
                count++;
                i++;
                j++;
            } else if(max < apartmentSize[j]) {
                i++;
            } else {
                j++;
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (t-- > 0) {
            ...
            int[] desiredApartmentSize = fs.readIntArray(n);
            int[] apartmentSize = fs.readIntArray(m);

            int count = distributeApartment(desiredApartmentSize, apartmentSize, k);
