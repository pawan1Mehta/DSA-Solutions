...
public class ConcertTickets {
    ...
    private static int[] getCustomersCost(int[] price, int[] maxPrice) {
        int n = price.length;
        int m = maxPrice.length;
        ...
        int[] res = new int[m];
        for(int i = 0; i < m; i++) {
            Map.Entry<Integer, Integer> data = mp.floorEntry(maxPrice[i]);
            ...
            if(data == null) {
                ...
                if(data.getValue() == 1) {
                    ...
                    mp.put(data.getKey(), data.getValue() - 1);
                }
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (t-- > 0) {
            ...
            int[] price = fs.readIntArray(n);
            int[] maxPrice = fs.readIntArray(m);

            int[] costs = getCustomersCost(price, maxPrice);

            for(int cost : costs) {
                fs.writer().write(cost + "\n");
            }
