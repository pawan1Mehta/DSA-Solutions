...
public class MovieFestival {
    private static int movieFestival(int[][] intervals) {
        ...
        int lastEntTime = intervals[0][1];
        for(int i = 1; i < n; i++) {
            if(intervals[i][0] < lastEntTime) {
                count++;
                lastEntTime = Math.min(lastEntTime, intervals[i][1]);
            } else {
                lastEntTime = intervals[i][1];
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (t-- > 0) {
            ...
            int[] interval = null;
            for(int i = 0; i < n; i++) {
                interval = fs.readIntArray(2);
                intervals[i] = interval;
            }

            int count = movieFestival(intervals);
