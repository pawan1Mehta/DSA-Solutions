package CSESProblemSet.Tree;

import java.io.*;
import java.util.*;

public class Subordinates {
    private static int dfs(int node, ArrayList<ArrayList<Integer>> adjList, boolean[] visited, int[] res) {
        ...
        for(int adjNode : adjList.get(node)) {
            if(!visited[adjNode]) {
                count += dfs(adjNode, adjList, visited, res);
            } else {
                count += res[adjNode];
            }
        }

        return res[node] = count + adjList.get(node).size();
    }

    private static int[] solve(int[] employee) {
