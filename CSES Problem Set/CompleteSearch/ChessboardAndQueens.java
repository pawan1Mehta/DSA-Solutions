package CSESProblemSet.CompleteSearch;
...
public class ChessboardAndQueens {
    ...
    private static boolean isValidCell(int row, int col, int[][] nums) {
        int n = 8;

        for(int i = 0; i < n; i++) {
            if(nums[i][col] == Integer.MAX_VALUE) {
                return false;
            }
        }
    ...
    private static long solve(int[][] nums, int row) {
        if(row == 8) {
            return 1;
        }
        ...
        for(int j = 0; j < 8; j++) {
            if(nums[row][j] == 1 && isValidCell(row, j, nums)) {
                nums[row][j] = Integer.MAX_VALUE;
                count += solve(nums, row + 1);
                nums[row][j] = 1;
            }
        }
