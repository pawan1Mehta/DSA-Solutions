...
public class AppleDivision {
    ...
    private static long minDiff(int i, long sum1, long sum2, int[] nums) {
        if(i == nums.length) {
            return Math.abs(sum1 - sum2);
        }

        return Math.min(
                minDiff(i + 1, sum1 + nums[i], sum2, nums),
                minDiff(i + 1, sum1, sum2 + nums[i], nums)
        );
    }

    private static long solve(int[] nums) {
        return minDiff(0, 0, 0, nums);
    }
